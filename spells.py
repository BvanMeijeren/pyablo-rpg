...
def fireball(caster,target,weapon):
    ...
    print_slow(caster.name +' used Fireball on ' + target.name + ' for ' + str(damage) + ' damage. ' + str(target.hitpoints) + ' HP remaining.')
...
spells = ['niffle','fireball','ice_shield']
#[k for k in dir() if '__' not in k]
