...
def niffle(mainchar,enemy,weapon):
    print_slow(enemy.name +' used Niffle! It hits you for ' + str(enemy.enemy_damage) + '.')
    mainchar.hitpoints = mainchar.hitpoints - enemy.enemy_damage

def fireball(mainchar,enemy,weapon):
    print_slow(enemy.name +' used Fireball! It hits you for ' + str(enemy.enemy_damage) + '.')
    mainchar.hitpoints = mainchar.hitpoints - enemy.enemy_damage

def ice_shield(mainchar,enemy,weapon):
    print_slow(enemy.name +' used Ice Shield! Damage reduced for 20%.')
    mainchar.hitpoints = mainchar.hitpoints - enemy.enemy_damage
